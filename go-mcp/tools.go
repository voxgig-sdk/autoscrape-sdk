...
type Args struct {
	Entity string         `json:"entity" jsonschema:"building_permit | business_entity | irs_990 | sec_edgar | stock_data | whoi | x402_paid"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AutoscrapeSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "autoscrape_list",
		Description: "List records from Autoscrape. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "autoscrape_load",
		Description: "Load a single record from Autoscrape. " +
...
func runOp(client *sdk.AutoscrapeSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AutoscrapeSDK, name string) (sdk.AutoscrapeEntity, error) {
	switch strings.ToLower(name) {
	case "building_permit":
		return client.BuildingPermit(nil), nil
	case "business_entity":
		return client.BusinessEntity(nil), nil
	case "irs_990":
		return client.Irs990(nil), nil
	case "sec_edgar":
		return client.SecEdgar(nil), nil
	case "stock_data":
		return client.StockData(nil), nil
	case "whoi":
		return client.Whoi(nil), nil
	case "x402_paid":
		return client.X402Paid(nil), nil
