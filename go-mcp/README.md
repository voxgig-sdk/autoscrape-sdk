...
[MCP](https://modelcontextprotocol.io) server exposing the Autoscrape SDK as
two agent tools — `autoscrape_list` and `autoscrape_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/autoscrape-mcp)
make build

# 2. Provide credentials via the environment
export AUTOSCRAPE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user autoscrape \
  -- /absolute/path/to/autoscrape-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./autoscrape-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// autoscrape_list: first page of records
{ "entity": "building_permit" }
{ "entity": "building_permit", "query": { } }

// autoscrape_load: one record by id
{ "entity": "building_permit", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/autoscrape-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user autoscrape \
     -- "$PWD"/dist/*/autoscrape-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `autoscrape_list` and `autoscrape_load` tools now appear
   in new sessions. Ask the agent to *"list building_permit using autoscrape"*
   and it calls `autoscrape_list` with `{"entity":"building_permit"}`.
...
```sh
export AUTOSCRAPE_APIKEY=sk_live_xxx            # API key
export AUTOSCRAPE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `autoscrape_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `autoscrape_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AUTOSCRAPE_APIKEY` | API key sent with every request. |
| `AUTOSCRAPE_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

building_permit | business_entity | irs_990 | sec_edgar | stock_data | whoi | x402_paid

### Smoke test via HTTP (raw JSON-RPC)

```sh
./autoscrape-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"autoscrape_load","arguments":{"entity":"building_permit","query":{"id":1}}}}'
```
