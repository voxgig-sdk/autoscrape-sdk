// Command Autoscrape-cli is a CLI + REPL for the Autoscrape SDK,
...
const prompt = "autoscrape"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "building_permit business_entity irs_990 sec_edgar stock_data whoi x402_paid"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AUTOSCRAPE_APIKEY (api key), AUTOSCRAPE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AUTOSCRAPE_APIKEY carries the API key and
	// AUTOSCRAPE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AUTOSCRAPE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AUTOSCRAPE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAutoscrapeSDK(opts)
