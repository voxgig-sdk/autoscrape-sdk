...
boru-driven command-line client **and** interactive REPL for the Autoscrape
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/autoscrape-cli)
make build
...
./autoscrape-cli load 1 building_permit            # {id:1} shorthand
./autoscrape-cli load '{id:1}' building_permit       # explicit match map

# 5. Override the API base URL for a single call
AUTOSCRAPE_BASE=https://api.example.com ./autoscrape-cli load 1 building_permit

# 6. No arguments -> interactive REPL
./autoscrape-cli
autoscrape> load 1 building_permit
autoscrape> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/autoscrape-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/autoscrape-cli load 1 building_permit
   ```
...
```sh
./autoscrape-cli load 1 building_permit          # scalar shorthand for {id:1}
./autoscrape-cli load '{id:1}' building_permit     # explicit match map
```
...
./autoscrape-cli load 1 building_permit
```
...
Run with no arguments to open a REPL (prompt `autoscrape>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `building_permit`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/autoscrape-cli`. |
...
(`./autoscrape-cli <expr>`) and interactively (the REPL), and expressions compose the
