...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("AUTOSCRAPE_TEST_LIVE") == "TRUE" ||
		os.Getenv("AUTOSCRAPE_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("AUTOSCRAPE_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.AutoscrapeSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.AutoscrapeError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.AutoscrapeSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.AutoscrapeError{Msg: msg, Code: code}
}
