...
import (
	...
	omni "github.com/voxgig-sdk/autoscrape-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AutoscrapeError); is {
				out := map[string]any{
					"name":    "AutoscrapeError",
...
func omniCtx(arg any, client *sdk.AutoscrapeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AutoscrapeSDK); is {
			client = live
