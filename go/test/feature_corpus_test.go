...
//
// NewAutoscrapeSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.AutoscrapeSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewAutoscrapeSDK(opts)
}
...
func fcCandidates(client *sdk.AutoscrapeSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.AutoscrapeSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.AutoscrapeSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.AutoscrapeError
