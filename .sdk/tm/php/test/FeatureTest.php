...
class FtCtrl extends AutoscrapeControl
{
...
class FtClient
{
    ...
    private AutoscrapeUtility $utility;

    public function __construct(AutoscrapeUtility $utility, array $options)
    {
    ...
    public function get_utility(): AutoscrapeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AutoscrapeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AutoscrapeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AutoscrapeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AutoscrapeConfig::make_feature($name);
    ...
    private static function buildUrl(AutoscrapeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AutoscrapeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AutoscrapeResponse($fetched);
        }
    ...
    private function fail(AutoscrapeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AutoscrapeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
