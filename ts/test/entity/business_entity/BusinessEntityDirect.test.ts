...
describe('BusinessEntityDirect', async () => {
  ...
  test('direct-load-business_entity', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-business_entity', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOSCRAPE_TEST_BUSINESS_ENTITY_ENTID']
