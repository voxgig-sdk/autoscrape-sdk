...
describe('BuildingPermitDirect', async () => {
  ...
  test('direct-load-building_permit', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-building_permit', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOSCRAPE_TEST_BUILDING_PERMIT_ENTID']
