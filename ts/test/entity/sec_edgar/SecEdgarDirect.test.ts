...
describe('SecEdgarDirect', async () => {
  ...
  test('direct-load-sec_edgar', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-sec_edgar', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOSCRAPE_TEST_SEC_EDGAR_ENTID']
