...
describe('X402PaidDirect', async () => {
  ...
  test('direct-load-x402_paid', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-x402_paid', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AUTOSCRAPE_TEST_X402_PAID_ENTID']
