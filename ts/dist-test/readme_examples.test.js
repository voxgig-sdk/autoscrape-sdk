...
const TEST_SEED = { "entity": { "building_permit": { "test01": { "id": "test01" } }, "business_entity": { "test01": { "id": "test01" } }, "irs_990": { "test01": { "id": "test01" } }, "sec_edgar": { "test01": { "id": "test01" } }, "stock_data": { "test01": { "id": "test01" } }, "whoi": { "test01": { "id": "test01" } }, "x402_paid": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+AutoscrapeSDK\b/.test(code) ||
        /\bAutoscrapeSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+AutoscrapeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/AutoscrapeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
