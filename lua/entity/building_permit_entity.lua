...
local BuildingPermitEntity = {}
BuildingPermitEntity.__index = BuildingPermitEntity


function BuildingPermitEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BuildingPermitEntity)
  self._name = "building_permit"
  self._client = client
...
function BuildingPermitEntity:get_name()
  return self._name
end


function BuildingPermitEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BuildingPermitEntity.new(self._client, opts)
end
...
function BuildingPermitEntity:mark_deleted()
  self._deleted = true
end


function BuildingPermitEntity:deleted()
  return true == self._deleted
end


function BuildingPermitEntity:data_set(args)
  if args ~= nil then
...
function BuildingPermitEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BuildingPermitEntity:match_set(args)
  if args ~= nil then
...
function BuildingPermitEntity:match_get()
...
function BuildingPermitEntity:stream(action, args, callopts)
...
---@param reqmatch BuildingPermitLoadMatch
---@param ctrl? table
---@return BuildingPermit
---@return string? err
function BuildingPermitEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function BuildingPermitEntity:_run_op(ctx, post_done)
  local utility = self._utility
