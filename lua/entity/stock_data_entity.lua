...
local StockDataEntity = {}
StockDataEntity.__index = StockDataEntity


function StockDataEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StockDataEntity)
  self._name = "stock_data"
  self._client = client
...
function StockDataEntity:get_name()
  return self._name
end


function StockDataEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StockDataEntity.new(self._client, opts)
end
...
function StockDataEntity:mark_deleted()
  self._deleted = true
end


function StockDataEntity:deleted()
  return true == self._deleted
end


function StockDataEntity:data_set(args)
  if args ~= nil then
...
function StockDataEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StockDataEntity:match_set(args)
  if args ~= nil then
...
function StockDataEntity:match_get()
...
function StockDataEntity:stream(action, args, callopts)
...
---@param reqmatch StockDataLoadMatch
---@param ctrl? table
---@return StockData
---@return string? err
function StockDataEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function StockDataEntity:_run_op(ctx, post_done)
  local utility = self._utility
