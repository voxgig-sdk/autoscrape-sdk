...
local X402PaidEntity = {}
X402PaidEntity.__index = X402PaidEntity


function X402PaidEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, X402PaidEntity)
  self._name = "x402_paid"
  self._client = client
...
function X402PaidEntity:get_name()
  return self._name
end


function X402PaidEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return X402PaidEntity.new(self._client, opts)
end
...
function X402PaidEntity:mark_deleted()
  self._deleted = true
end


function X402PaidEntity:deleted()
  return true == self._deleted
end


function X402PaidEntity:data_set(args)
  if args ~= nil then
...
function X402PaidEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function X402PaidEntity:match_set(args)
  if args ~= nil then
...
function X402PaidEntity:match_get()
...
function X402PaidEntity:stream(action, args, callopts)
...
---@param reqmatch X402PaidLoadMatch
---@param ctrl? table
---@return X402Paid
---@return string? err
function X402PaidEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function X402PaidEntity:_run_op(ctx, post_done)
  local utility = self._utility
