...
// Autoscrape SDK BusinessEntity entity
...
class BusinessEntityEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new BusinessEntityEntity($this->_client, $opts);
    }

    /**
     * @param BusinessEntity|array $args BusinessEntity data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = AutoscrapeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return BusinessEntity|array The current BusinessEntity data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of BusinessEntity fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = AutoscrapeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of BusinessEntity fields).
     */
    ...
    /**
     * Load a single BusinessEntity.
     *
     * @param BusinessEntityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed BusinessEntityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return BusinessEntity|array The loaded BusinessEntity as an assoc-array at the
     *   SDK boundary; throws AutoscrapeError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = AutoscrapeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
