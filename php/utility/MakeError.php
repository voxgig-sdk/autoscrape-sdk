...
class AutoscrapeMakeError
{
    public static function call(?AutoscrapeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AutoscrapeContext([], null);
        }
        $op = $ctx->op ?? new AutoscrapeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AutoscrapeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AutoscrapeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AutoscrapeError('', $msg, $ctx);
        ...
        if ($err instanceof AutoscrapeError) {
