...
class AutoscrapeNetsimFeature extends AutoscrapeBaseFeature
{
    ...
    private function _simulate(AutoscrapeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(AutoscrapeContext $ctx, array $applied): void
    {
        $client = $this->client;
