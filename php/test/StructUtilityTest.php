...
// Autoscrape SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AutoscrapeOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AutoscrapeOmni::structfix($now, Struct::undef()))
                        !== json_encode(AutoscrapeOmni::structfix($val, Struct::undef()))) {
