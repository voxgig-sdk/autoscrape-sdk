...
{
    private static function utility(): AutoscrapeUtility
    {
        return new AutoscrapeUtility();
    }
    ...
    private static function ctx(array $over = []): AutoscrapeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AutoscrapeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AutoscrapeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AutoscrapeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AutoscrapeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AutoscrapeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AutoscrapeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AutoscrapeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AutoscrapeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AutoscrapeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AutoscrapeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AutoscrapeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AutoscrapeResponse([]),
            'result' => new AutoscrapeResult([]),
        ]);
        [, $err] = AutoscrapeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AutoscrapeSpec([]),
            'response' => null,
            'result' => new AutoscrapeResult([]),
        ]);
        [, $err] = AutoscrapeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AutoscrapeSpec(['step' => 's']),
            'response' => new AutoscrapeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AutoscrapeResult([]),
        ]);
        [, $err] = AutoscrapeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AutoscrapeSpec(['step' => 's']),
            'response' => new AutoscrapeResponse(self::resp(200, ['v' => 1])),
            'result' => new AutoscrapeResult([]),
        ]);
        [, $err] = AutoscrapeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AutoscrapeSpec(['step' => 's']),
            'response' => new AutoscrapeResponse(self::resp(200, ['v' => 2])),
            'result' => new AutoscrapeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AutoscrapeMakeResponse::call($ctx);
    ...
    {
        $preset = new AutoscrapeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AutoscrapeSpec([]),
            'response' => new AutoscrapeResponse([]),
            'result' => new AutoscrapeResult([]),
        ...
        [$got, $err] = AutoscrapeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AutoscrapeResult([])]);
        [, $err] = AutoscrapeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AutoscrapeSpec([]), 'result' => null]);
        [, $err] = AutoscrapeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AutoscrapeSpec(['step' => 's']),
            'result' => new AutoscrapeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AutoscrapeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AutoscrapeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AutoscrapeSpec(['step' => 's']),
            'result' => new AutoscrapeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AutoscrapeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AutoscrapeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AutoscrapeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AutoscrapeSpec([]),
            'result' => new AutoscrapeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AutoscrapeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AutoscrapeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AutoscrapeError('boom', 'boom');
        $utility->fetcher = function (AutoscrapeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AutoscrapeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AutoscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AutoscrapeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AutoscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AutoscrapeResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AutoscrapeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AutoscrapeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AutoscrapeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AutoscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AutoscrapeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AutoscrapeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AutoscrapeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AutoscrapeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AutoscrapeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AutoscrapeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AutoscrapeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AutoscrapeResult(['ok' => false])]);
        $this->expectException(AutoscrapeError::class);
        AutoscrapeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AutoscrapeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AutoscrapeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AutoscrapeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AutoscrapeBaseFeature();
        $b = new AutoscrapeBaseFeature();
        AutoscrapeFeatureAdd::call($ctx, $a);
        AutoscrapeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AutoscrapeBaseFeature
    {
        $f = new AutoscrapeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AutoscrapeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AutoscrapeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AutoscrapeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AutoscrapePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AutoscrapePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AutoscrapeResponse(['headers' => null]),
            'result' => new AutoscrapeResult([]),
        ]);
        AutoscrapeResultHeaders::call($ctx);
