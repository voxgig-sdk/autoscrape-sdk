# Autoscrape Python SDK



The Python SDK for the Autoscrape API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.BuildingPermit()` — each
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/autoscrape-sdk/releases)) or
...
```python
stock_data = client.StockData().load({"symbol": "symbol"})
```
...
```
py/
├── autoscrape_sdk.py         -- Main SDK module
