...
class TestBusinessEntityEntity:

    def test_should_create_instance(self):
        testsdk = AutoscrapeSDK.test(None, None)
        ent = testsdk.BusinessEntity(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _business_entity_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "business_entity." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AUTOSCRAPE_TEST_BUSINESS_ENTITY_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        business_entity_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.business_entity")))
        business_entity_ref01_data = None
        if len(business_entity_ref01_data_raw) > 0:
            business_entity_ref01_data = helpers.to_map(business_entity_ref01_data_raw[0][1])

        # LOAD
        business_entity_ref01_ent = client.BusinessEntity(None)
        business_entity_ref01_match_dt0 = {}
        business_entity_ref01_data_dt0_loaded = business_entity_ref01_ent.load(business_entity_ref01_match_dt0, None)
        assert business_entity_ref01_data_dt0_loaded is not None



def _business_entity_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/business_entity/BusinessEntityTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "AUTOSCRAPE_TEST_BUSINESS_ENTITY_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("AUTOSCRAPE_TEST_BUSINESS_ENTITY_ENTID"))
