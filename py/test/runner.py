...
class AutoscrapeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AutoscrapeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AutoscrapeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AutoscrapeTestRunner.getenv("AUTOSCRAPE_TEST_LIVE")
        override = AutoscrapeTestRunner.getenv("AUTOSCRAPE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AutoscrapeTestRunner.getenv(key)
        ...
        explain = AutoscrapeTestRunner.getenv("AUTOSCRAPE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["AUTOSCRAPE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AutoscrapeTestRunner._test_control is not None:
            return AutoscrapeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AutoscrapeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AutoscrapeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AutoscrapeTestRunner.load_test_control()
...
def load_env_local():
    AutoscrapeTestRunner.load_env_local()


def env_override(m):
    return AutoscrapeTestRunner.env_override(m)


def entity_data(v):
    return AutoscrapeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AutoscrapeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AutoscrapeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AutoscrapeTestRunner.load_test_control()


def live_client_options():
    return AutoscrapeTestRunner.live_client_options()


def live_delay_ms():
    return AutoscrapeTestRunner.live_delay_ms()
