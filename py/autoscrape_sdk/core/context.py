...
from autoscrape_sdk.core.control import AutoscrapeControl
...
from autoscrape_sdk.core.helpers import get_ctx_prop, to_map
...
class AutoscrapeContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, AutoscrapeSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, AutoscrapeResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, AutoscrapeResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return AutoscrapeError(code, msg, self)
