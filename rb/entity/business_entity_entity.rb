...
class BusinessEntityEntity
  ...
  def make
    opts = @_entopts.dup
    BusinessEntityEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AutoscrapeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [BusinessEntity, Hash] the current BusinessEntity data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = AutoscrapeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of BusinessEntity fields)
  def match_get
  ...
  #
  # @param reqmatch [BusinessEntityLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.BusinessEntity.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [BusinessEntity, Hash] the loaded BusinessEntity; raises AutoscrapeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = AutoscrapeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
