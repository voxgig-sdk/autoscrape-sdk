...
class X402PaidEntity
  ...
  def make
    opts = @_entopts.dup
    X402PaidEntity.new(@_client, opts)
  end
  ...
  # @return [X402Paid, Hash] the current X402Paid data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of X402Paid fields)
  def match_get
  ...
  #
  # @param reqmatch [X402PaidLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.X402Paid.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [X402Paid, Hash] the loaded X402Paid; raises AutoscrapeError on failure
