...
class X402PaidEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set AUTOSCRAPE_TEST_X402_PAID_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def x402_paid_basic_setup(extra)
  ...
  entid_env_raw = ENV["AUTOSCRAPE_TEST_X402_PAID_ENTID"]
