...
module AutoscrapeUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = AutoscrapeContext.new({}, nil)
    end
    op = ctx.op || AutoscrapeOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || AutoscrapeResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(AutoscrapeError) ? err.msg : err.to_s
    ...
    sdk_err = AutoscrapeError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(AutoscrapeError)
