...
module AutoscrapeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AutoscrapeResponse.new({})
    ...
    if fetch_err
      ...
      response = AutoscrapeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AutoscrapeResponse.new(fetched)
    else
